...
### 3.3 Bazel Rules and other bazel native integrations

Custom bazel rules can be created to encapsulate tool invocations and configurations, providing a more integrated experience.

Pros:
* Can leverage bazel caching and remote execution
* Can be used where aspects or multitool is not suitable

Cons:
* Increased complexity in managing custom rules
...
* All tools are provided via devcontainer.
* We use said devcontainer in CI as well (stripped down etc).
* In addition we use bazel aspects for tools that benefit from bazel-config-dependent analysis or caching. Custom bazel rules are also a possibility, but are currently not actively pursued.
* Especially for small and fast CI jobs, we use native installs, as any bazel or devcontainer invocation is simply too slow (see deep dive chapter 4.3).
...
*Ideally we provide these **via bazel aspects and devcontainer** for IDE support*
...
*Ideally we provide these **via devcontainer***
...
### System dependencies
*These cannot be provided by bazel. We must provide these **via devcontainer***
