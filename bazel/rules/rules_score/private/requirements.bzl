...
def _requirements_impl(ctx):
    ...
    if ctx.attr.req_kind == "feature":
        system_req_lobster_depsets = []
        for dep in ctx.attr.deps:
            if AssumedSystemRequirementsInfo in dep:
                system_req_lobster_depsets.append(dep[AssumedSystemRequirementsInfo].srcs)
        req_provider = FeatureRequirementsInfo(
            srcs = depset([lobster_file]),
            system_req_srcs = depset(transitive = system_req_lobster_depsets),
            name = ctx.label.name,
        )
