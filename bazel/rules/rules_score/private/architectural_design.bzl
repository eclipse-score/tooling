...
def _architectural_design_impl(ctx):
    ...
    all_source_files = depset(
        transitive = [
            depset(_colocate_puml_with_wrapper(ctx, ctx.files.static, ctx.label.name + "/static", stems)),
            depset(_colocate_puml_with_wrapper(ctx, ctx.files.dynamic, ctx.label.name + "/dynamic", stems)),
            depset(_colocate_puml_with_wrapper(ctx, ctx.files.public_api, ctx.label.name + "/public_api", stems)),
            depset(_colocate_puml_with_wrapper(ctx, ctx.files.internal_api, ctx.label.name + "/internal_api", stems)),
            depset(_colocate_puml_with_wrapper(ctx, ctx.files.static_view, ctx.label.name + "/static_view", stems)),
        ],
    )
    ...
    # Generate path-preserving wrappers and directory index pages per view.
    # Each view gets its own output subdirectory so index.rst files for
    # different views (which may share the same relative source directory)
    # don't collide. The root index of each view is the toctree entry used
    # by the dependable_element index (directly, or via the categorized
    # software_arch page); nested indexes and wrappers are reached through
    # its own toctrees.
    static_navigation = make_puml_rst_navigation(
        ctx,
        ctx.files.static,
        ctx.label.name + "/static",
        ctx.file._puml_rst_template,
        stems = stems,
    )
    dynamic_navigation = make_puml_rst_navigation(
        ctx,
        ctx.files.dynamic,
        ctx.label.name + "/dynamic",
        ctx.file._puml_rst_template,
        stems = stems,
    )
    public_api_navigation = make_puml_rst_navigation(
        ctx,
        ctx.files.public_api,
        ctx.label.name + "/public_api",
        ctx.file._puml_rst_template,
        stems = stems,
    )
    internal_api_navigation = make_puml_rst_navigation(
        ctx,
        ctx.files.internal_api,
        ctx.label.name + "/internal_api",
        ctx.file._puml_rst_template,
        stems = stems,
    )
    static_view_navigation = make_puml_rst_navigation(
        ctx,
        ctx.files.static_view,
        ctx.label.name + "/static_view",
        ctx.file._puml_rst_template,
        stems = stems,
    )

    def _get_doc_files(files, navigation):
        docs = [f for f in files if f.extension in ("rst", "md")]
        root = [navigation.root_index] if navigation.wrappers else []
        return depset(docs + root)

    static_doc_files = _get_doc_files(ctx.files.static, static_navigation)
    dynamic_doc_files = _get_doc_files(ctx.files.dynamic, dynamic_navigation)
    public_api_doc_files = _get_doc_files(ctx.files.public_api, public_api_navigation)
    internal_api_doc_files = _get_doc_files(ctx.files.internal_api, internal_api_navigation)
    static_view_doc_files = _get_doc_files(ctx.files.static_view, static_view_navigation)
    ...
    all_navigations = [static_navigation, dynamic_navigation, public_api_navigation, internal_api_navigation, static_view_navigation]
    sphinx_srcs = depset(
        transitive = [static_doc_files, dynamic_doc_files, public_api_doc_files, internal_api_doc_files, static_view_doc_files],
    )
    sphinx_aux_srcs = depset([f for nav in all_navigations for f in nav.wrappers + nav.indexes])
