...
from test_case_coverage.compute_lock import LockFile, RequirementEntry, TestCase
...
def _tc(uid="Suite:A", given="g", when="w", then="t", gtest_tag=""):
    return TestCase(uid=uid, given=given, when=when, then=then, gtest_tag=gtest_tag)
...
def test_generates_valid_lobster_json(tmp_path):
    ...
    assert data["generator"] == "test_case_coverage"
...
def test_one_activity_per_requirement(tmp_path):
    ...
    assert len(tags) == 2
    assert set(tags) == {"test_case_coverage P.A", "test_case_coverage P.B"}
...
def test_uncovered_test_has_drift_message(tmp_path):
    ...
    assert any("test case coverage not confirmed" in m for m in item["messages"])
...
def test_tracing_ref_points_to_each_gtest_item(tmp_path):
    """Every test case with a gtest_tag gets a native cross-reference to its
    underlying gtest lobster item, so the rendered report can link to it.
    """
    tc_a = _tc(uid="//pkg/Suite:A", gtest_tag="Suite:A")
    tc_b = _tc(uid="//pkg/Suite:B", gtest_tag="Suite:B")
    computed = _make_lock(_req("P.R", tc_a, tc_b))
    out = tmp_path / "out.lobster"
    generate_lobster(computed, None, "//pkg:cov", out)
    item = json.loads(out.read_text())["data"][0]
    assert "gtest Suite:A" in item["refs"]
    assert "gtest Suite:B" in item["refs"]


def test_no_gtest_ref_when_gtest_tag_missing(tmp_path):
    computed = _make_lock(_req("P.R", _tc(gtest_tag="")))
    out = tmp_path / "out.lobster"
    generate_lobster(computed, None, "//pkg:cov", out)
    item = json.loads(out.read_text())["data"][0]
    assert not any(ref.startswith("gtest ") for ref in item["refs"])


def test_item_location_is_always_void(tmp_path):
    """The aggregate Activity has no single canonical source once it groups
    multiple test cases, so its own location always stays void; the real
    per-test source is reachable via the gtest cross-references instead.
    """
    computed = _make_lock(_req("P.R", _tc(uid="S:A", gtest_tag="S:A"), _tc(uid="S:B", gtest_tag="S:B")))
    out = tmp_path / "out.lobster"
    generate_lobster(computed, None, "//pkg:cov", out)
    item = json.loads(out.read_text())["data"][0]
    assert item["location"]["kind"] == "void"
...
def test_partial_drift_in_one_test_case_fails_whole_requirement(tmp_path):
...
def test_not_covered_message_includes_label(tmp_path):
    computed = _make_lock(_req("P.R", _tc()))
    out = tmp_path / "out.lobster"
    generate_lobster(computed, None, "//pkg:cov", out)
    item = json.loads(out.read_text())["data"][0]
    assert any("//pkg:cov" in m for m in item["messages"])
