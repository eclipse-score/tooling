...
1. `@docs_runtime//:flat` provides a distroless rootfs tar (from the
   `apt.install(name = "docs_runtime", manifest = "//third_party/docs_runtime:manifest.yaml", ...)`
   tag in `//MODULE.bazel`) containing `graphviz` and `fakechroot`.
...
Edit the `packages` list in `manifest.yaml`, then re-run any build that
depends on `@docs_runtime` (rules_distroless resolves and re-locks the
closure automatically since `nolock = True` is set on the `apt.install(...)`
tag -- no separate lock file to regenerate/commit).
