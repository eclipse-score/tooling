...
pub enum IdMapModel<'a> {
    ...
    /// Diagrams with no cross-linkable elements (e.g. activity); produces an
    /// empty idmap so the parser's always-declared sidecar output is written.
    Empty,
}
...
fn comp_model_to_idmap(model: &HashMap<String, LogicComponent>, source: &str) -> IdMapFile {
    ...
    for comp in model.values() {
        let alias = comp
            ...
            .unwrap_or(&comp.id)
            .to_string();
        let is_define = has_children.contains(comp.id.as_str());
...
pub fn write_idmap_to_file(
    ...
    let idmap = match model {
        IdMapModel::Component(m) => comp_model_to_idmap(m, &source),
...
mod tests {
    ...
    fn component_children_make_define_leaves_make_reference() {
        ...
        let idmap = comp_model_to_idmap(&model, "pkg/proxy.puml");
    ...
    fn component_with_no_children_is_all_references() {
        ...
        let idmap = comp_model_to_idmap(&model, "pkg/overview.puml");
    ...
    fn component_alias_falls_back_to_name_then_id() {
        ...
        let idmap = comp_model_to_idmap(&model, "pkg/aliases.puml");
    ...
    fn component_output_is_sorted_by_id() {
        ...
        let idmap = comp_model_to_idmap(&model, "pkg/sorted.puml");
